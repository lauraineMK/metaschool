...
<div class="container py-5">
    <h1 class="fw-bold mb-4" style="color: #7C3AED; font-family: 'Inter', sans-serif; letter-spacing: 1px;">Modifier le cours</h1>
    <form method="POST" action="{{ route('admin.courses.update', $course->id) }}">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label for="name" class="form-label fw-semibold">Titre du cours</label>
            <input type="text" class="form-control rounded-3" id="name" name="name" value="{{ $course->name }}" required>
        </div>
        <div class="mb-3">
            <label for="description" class="form-label fw-semibold">Description</label>
            <textarea class="form-control rounded-3" id="description" name="description" rows="3">{{ $course->description }}</textarea>
        </div>
        <div class="mb-3">
            ...
            <select class="form-select rounded-3" id="author_id" name="author_id" required>
                @foreach(\App\Models\User::whereIn('role', ['teacher', 'admin'])->get() as $user)
                    <option value="{{ $user->id }}" @if($user->id == $course->author_id) selected @endif>{{ $user->firstname }} {{ $user->lastname }} ({{ $user->role }})</option>
                @endforeach
            </select>
        </div>
